...
import com.example.miruking.activities.Routine;
    ...
    public void onViewCreated(@NonNull View view, Bundle savedInstanceState) {
        ...
        fab.setOnClickListener(fabView -> {
            ...
            popupMenu.setOnMenuItemClickListener(item -> {
                ...
                switch (type) {
                    case "일반":
                        // ⬇️ 4개 인자( Todo, 날짜, ScheduleType, 콜백 )로 한 번만 호출
                        dialogManager.showScheduleDialog(
                                (Todo) null,                                  // 새 생성이면 null
                                selectedDate,
                                ScheduleDialogManager.ScheduleType.CREATE,    // ✅ 실제 enum에 맞게 변경 (예: NORMAL/NEW 등)
                                newTodoId -> {
                                    loadTodosForDate(selectedDate);
                                    // 알림 트리거 (원래 바깥에서 하던 로직 여기로 이동)
                                    if (!NotificationTracker.getSentTodaySet(requireContext())
                                            .contains(String.valueOf(newTodoId))) {
                    ...
                    case "D-Day":
                        // D-Day 전용 다이얼로그 (메서드 시그니처에 맞게)
                        dialogManager.showDdayDialog(
                                (Todo) null,
                                selectedDate,
                                newTodoId -> {
                                    loadTodosForDate(selectedDate);
                    ...
                    case "루틴":
                        // 루틴 전용 다이얼로그
                        dialogManager.showRoutineDialog(
                                (Routine) null,
                                selectedDate,
                                newTodoId -> {
                                    loadTodosForDate(selectedDate);
